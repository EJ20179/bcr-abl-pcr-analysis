...
### BR-08. Previous study 보조 입력 (OCR)
- Work List 카드에서 직전 결과지 이미지를 클립보드에서 붙여넣기(Ctrl+V)로 입력
- OCR(Tesseract.js)로 BCR::ABL1 %IS 값, Molecular response 자동 추출
- 접수 날짜(달력 picker)·검체 종류(EDTA WB/BM 토글) 선택 후 Previous study 텍스트 자동 생성
- 구성: 줄1 `(날짜)-{검체} : {%IS} / {MR약칭}` + 이전 보고서 Previous study 항목 최대 2줄
- 날짜 상한: 검체 ID 앞 6자리(YYMMDD)를 자동 파싱하여 달력 picker의 최대 날짜로 설정 — 현재 검사 접수일 이후 날짜 선택 불가
- 개인정보 보호: 환자 정보 상단 제외, 결과표·Remark 영역만 캡쳐하여 입력
